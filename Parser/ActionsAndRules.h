...
{  
  ...
  {
  public:
    using Rslt = Logger_code_publisher::StaticLogger<0>;  // use for application results
    using Demo = Logger_code_publisher::StaticLogger<1>;  // use for demonstrations of processing
    using Dbug = Logger_code_publisher::StaticLogger<2>;  // use for debug output
  ...
  class PrintFunction : public IAction
  {
    Repository* p_Repos;
  public:
    using Rslt = Logger_code_publisher::StaticLogger<0>;
  ...
  */
  ...
  /////////////////////////////////////////////////////////////
  // Rule to display comments
  class Comments : public IRule
  {
  public:
	  bool doTest(const Lexer::ITokenCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Test comments", *pTc);

		  // don't parse Semi with single semicolon token

		  if (pTc->contains("//") || pTc->contains("/*"))
		  {
  ...
  class HandleComments : public IAction
  {
	  Repository* p_Repos;
  public:
	  HandleComments(Repository* pRepos)
	  {
		  p_Repos = pRepos;
	  }
	  void doAction(const Lexer::ITokenCollection* pTc) override
