...
void GrammarHelper::showParse(const std::string& msg, const Lexer::ITokenCollection& se, bool isResult)
{
  ...
  using Dbug = Logger_code_publisher::StaticLogger<1>;/////////////// changed 2 to 1
...
{
  using Demo = Logger_code_publisher::StaticLogger<0>;
  if ((Demo::running()))////////changed 1 t0 2
  {
    std::ostringstream out;
...
void testHelpers(const Semi& seIn)
{
  ...
  Utilities_code_publisher::StringHelper::title("Testing GrammarHelpers");
  std::cout << "\n  " << show(se);
  se.trim();
...
int main()
{
  Utilities_code_publisher::StringHelper::Title("Testing Grammar Functions");
  std::cout << "\n  Note:";
