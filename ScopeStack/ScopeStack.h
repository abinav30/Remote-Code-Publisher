...
namespace CodeAnalysis
{
  ...
  class ScopeStack {
  public:
    using Rslt = Logger_code_publisher::StaticLogger<0>;    // show program results
    using Demo = Logger_code_publisher::StaticLogger<1>;    // show demonstration outputs
    using Dbug = Logger_code_publisher::StaticLogger<2>;    // show debugging outputs
  ...
  element ScopeStack<element>::pop()
  {
    ...
    stack.pop_back();    
   Rslt::write("\n--- stack size = " + Utilities_code_publisher::Converter<size_t>::toString(size()) + " ---");
    Dbug::flush();
  ...
  template<typename element>
  void showStack(ScopeStack<typename element>& stack, bool indent = true)
  {
    if (stack.size() == 0)
    {
      std::cout << "\n  ScopeStack is empty";
  ...
  void TreeWalk(element* pItem, bool details = false)
  {
    ...
    ++indentLevel;
	/*
	auto it = pItem->statements_.begin();
	while (it != pItem->statements_.end()) {
		std::cout << "\n  " << std::string(2 * indentLevel, ' ') << (*it)->ToString();
		++it;
	}
	*/
